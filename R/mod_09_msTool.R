      ...
      column(width = 4,
             box(
               ...
               fileInput(inputId = ns("formula2"),
                         ...
                         multiple = FALSE,
                         placeholder = "should contain a column called Formula",
                         accept = c(".csv",
                                    ".xls",
                                    ".xlsx"
                                    )
  ...
  moduleServer( id, function(input, output, session){
    ...
    adductTypes <- reactive({
      if(is.null(ionMode1())){
        ...
        return(c("H", "CH2O2", "CH3COOH", "HPO3", "HSO3"))
      } else{
    ...
    observeEvent(input$searchNoise, {
      ...
      output$noiseOutput <- DT::renderDataTable({
        ...
        shiny::withProgress(
          ...
          value = 0.4,
          {
            noiseList <- MSbox::contam(mz = noiseList,
                                       mode = ionMode3(),
                                       ppm = ppmTolerance2())
          })

        if(is.null(noiseList)){noiseList = data.frame(Result = "Not Found")}
        DT::datatable(noiseList,
                      rownames = FALSE,
                      options = list(scrollX = TRUE,
                                     deferRender = TRUE,
                                     scroller = TRUE,
