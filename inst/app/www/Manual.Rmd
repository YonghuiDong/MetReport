...
If your sample-feature table was generated by Compound Discoverer software, then `PeakArea of Sample1` can be also modified as `WT_Male_1`. The keyword **Sample** is not necessary in this case, but you can keep it as well.
...
We have provided two demo datasets, prepared using method 1 and method 2, respectively. Please go to the **Home** Tab of MetaboReport Webapp to download them.

You can refer to the two demo data to help you prepare your own dataset.

# Common Mistakes

**1. My data table is not accepted or not recognized by MetaboReport.**

99% of the mistakes are resulted from the inappropriate input data format. Please pay attention to the following points when preparing your data table:

> - Metadata is case sensitive, therefore **WT**, **Wt** and **Wt**, and **wt** will be treated as different metadata, so make sure your metadata are consistent. 
> 
...
>
> - Pay attention to **white spaces**. `WT` and <code>WT&nbsp;</code> with one space will be treated as different metadata.
> 
> - If your input data table is generated by **Compound Discoverer**, please make sure you have exported all the "Area" columns, which contain peak area information; Please also select `Compound Discoverer(CD)`format when uploading the data in **Upload Data** tab. If your data table is produced with other software tools, please select `Other Format`.

**2. It takes too long to generate the report.**

> - Some additional calculations are being performed during report generation, it may take few minutes to finish.
> 
> - If the report generation is stuck, you may have to restart the PC and re-run your analysis.
