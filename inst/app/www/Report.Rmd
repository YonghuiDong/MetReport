...
Metabolomics is the systematic study of low molecular weight molecules (i.e., metabolites) in a given biological system. Depending on the research question or specific application, there are three common metabolomics strategies [1]:
...
This report is an overview of your data analysis results. Brief explanations are provided in each section to help you better understand the report. Below are few tips on how to use the report.
...
>
> (2) High resolution (600 dpi) static figures are provided along with this report. You can also download them in `Statistics` tab if you are using **MetaboReport** by yourself to generate this report. 
>
...
plotly::ggplotly(params$myPCAPlot, tooltip = "label")
```
...
DT::datatable(myRawData,
              ...
              options = list(scrollX = TRUE,
                             ...
                             initComplete = JS(
                                ...
                                "}"),
                             ## use scientific notation (This is only applied to numeric columns)
                              rowCallback = JS(
                                "function(row, data) {",
...
>
> Because metabolomics data can be acquired from various platforms and/or pre-processed with different software tools, the resulting peak table can be very different. Here we highlight few common terms that may appear in the table.
>
> `m/z`: the mass-to-charge ratio of an ion is the number obtained by dividing the mass of the ion (m) by the number of electrical charges (z). It is closely related to the identity of a metabolite. Note that m/z is different from molecular weight.
>
> `RT`: retention time is the amount of time a metabolite spends on the column after it has been injected. It is a parameters that related to metabilite identity.
>
...
**Table 2. Data table with statistical information**
...
> <span style = "color : #FF8300">**How to interpret Table 2?**</span>
>
> **(1) Parameters in Table 2 **
>
...
> 
> Our suggested values are: mass features with adjusted p-value < 0.05 and VIP >= 1 are considered as statistically signinificantly different, and FC >= 2 as increase and FC <= 0.05 as decrease. See **Table 3** below.
...
## prepare data
OPLSDALevel <- sfData$group[, OPLSDAGroup]
...
for (i in seq_along(listnames)){
  ...
  p1 <- MetaboReport::showOPLSDA(resultOPLSDA, Group = groupOPLSDA, size = 1, interactive = TRUE)
  p2 <- MetaboReport::showSplot(matrixOPLSDA, resultOPLSDA, size = 1, interactive = TRUE)
...
VCGroupLevel <- sfData$group[, VCGroup]
...
for (i in seq_along(listnames)){
    volcanoList[[i]] <- plotly::as_widget(
      MetaboReport::showVolcano(result = statTable, compare_group = listnames[[i]], FC = 2, p = 0.05)
      )
    }
...
The `Circular Dendrogram` gives an overview of the number of mass features in each subset (subgroup).
...
The bar (violin or box) plot shows the quantitative information of each metabolite in different sample group.

> <span style = "color : #FF8300">**Note**</span>
>
> Depending on the setting in **MetaboReport** workflow, barplot, violin or box plot is displayed below for each metabolite.
>
...
yText <- switch(params$BPTransform,
               none = "Raw Peak Area",
               log2 = "Log2 Transformed Peak Area",
               log10 = "Log10 Transformed Peak Area"
               )

BPPlot <- ggplot2::ggplot(data = BPData, 
                          aes(x = Group, 
                              y = Area, 
                              group = interaction(Metabolite, Group)
                              )
                          ) +
  ggplot2:: geom_boxplot(aes(fill = Group)) +
  ggplot2::theme_bw() +
  ggplot2::ylab(yText)

filter <- crosstalk::bscols(
  metaboliteFilter,
  plotly::ggplotly(BPPlot, tooltip = NULL),
  widths = 12
  )
